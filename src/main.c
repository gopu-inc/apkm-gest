...
int install_package(const char *name, const char *version) {
    ...
    if (search_package(name, ver, url) != 0) {
        printf("[APKM] [x] Package %s not found\n", name);
        return -1;
    }
    
    printf("[APKM] [v] Found version %s\n", ver);
    ...
    if (download_package(url, tmp_path) != 0) {
        printf("[APKM] [x] Download failed\n");
        return -1;
    }
    
    printf("[APKM] [p] Installing...\n");
    
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "tar -xzf %s -C /usr/local/bin 2>/dev/null", tmp_path);
    
    if (system(cmd) == 0) {
        printf("[APKM] [v] Package %s installed successfully\n", name);
        unlink(tmp_path);
        return 0;
    }
    ...
    printf("[APKM] [x] Installation failed\n");
    unlink(tmp_path);
    return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "install") == 0) {
        if (argc < 3) {
            printf("[APKM] [x] Missing package name\n");
            return 1;
        }
        ...
        if (argc < 3) {
            printf("[APKM] [x] Missing search term\n");
            return 1;
        }
        char ver[64], url[512];
        if (search_package(argv[2], ver, url) == 0) {
            printf("[APKM] [v] Package %s found (version %s)\n", argv[2], ver);
        } else {
            printf("[APKM] [x] Package %s not found\n", argv[2]);
        }
        return 0;
        ...
        printf("[APKM] [x] Unknown command: %s\n", argv[1]);
        print_help();
        return 1;
    }
